...
int frameprocessing(int argc, char* argv[]) {
	cv::Mat frame = cv::imread(imagesrc);
...
int videoprocessing(int argc, char* argv[]) {
...
int main(int argc, char* argv[]) {
	...
	if (source != "default" && imagesrc != "default") {
		std::cout << "Can't provide both a video source and an image source, exiting... " << std::endl;
		return 1;
	}

	if (source == "default" && imagesrc == "default")
		successcode = videoprocessing(argc, argv);
	else if (source != "default")
		successcode = videoprocessing(argc, argv);
	else successcode = frameprocessing(argc, argv);
